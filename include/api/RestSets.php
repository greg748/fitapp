...
users
    GET /sets/types - get a list of set types
    GET /sets/units - get a list of set units
    GET /sets/workout/:id - get sets for a particular workout instance
    POST /sets - create a set
    
sets/:id
    GET /sets/:id - get a particular set
    PUT /sets/:id - update a set record
    DELETE /sets/:id - delete the set record

*/

class RestSets extends RestService {
    ...
    public function performGet($arguments, $accept) {
        ...
        if ($arguments['workout'] > 0) {
            ...
            if ($WorkoutInstance) {
                $data = $WorkoutInstance->getWorkoutData();
            } else {
                $success = false;
