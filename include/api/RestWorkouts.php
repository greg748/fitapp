...
class RestWorkouts extends RestService {
    protected $supportedMethods = ['GET', 'POST', 'PUT', 'OPTIONS', 'DELETE'];
    protected $nounsList = ['workouts','users', 'status','trainer'];
    ...
    public function performGet($arguments, $accept) {
        ...
        if ($arguments['users'] == NULL) {
            ...
            $message = "Get User workouts $users";
            $users = Workouts::getUserWorkouts($arguments['users']);
        }
        $data = $users;
